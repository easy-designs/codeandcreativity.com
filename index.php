 ...
 *
 */
 $assign_to_config['site_name']  = 'code-and-creativity'; 
 //  $assign_to_config['cp_url'] = 'http://domain2.com/admin.php'; 
 $assign_to_config['site_url'] = "//{$_SERVER['SERVER_NAME']}"; 
 $assign_to_config['site_index'] = '';

 // Global Vars
 $assign_to_config['global_vars']['site_domain'] = $_SERVER['SERVER_NAME'];
 // CDN  
 $assign_to_config['global_vars']['cdn_domain'] = '//e.ed-cdn.com';
 // Local only
 #$assign_to_config['global_vars']['cdn_domain'] = '';

 # CDN GZip available?
 
 $curl = curl_init();
 curl_setopt($curl, CURLOPT_URL, 'http://e.ed-cdn.com/css/main.gz.css');
 curl_setopt($curl, CURLOPT_HEADER, true);
 curl_setopt($curl, CURLOPT_NOBODY, true);
 curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
 $headers = curl_exec($curl);
 curl_close($curl);
 $assign_to_config['global_vars']['gzip_ext'] = ( strpos( $headers, 'Content-Encoding: gzip' ) !== false ) ? 'gz.' : '';
